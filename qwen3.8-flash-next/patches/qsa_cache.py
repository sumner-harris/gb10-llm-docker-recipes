...
class QSAKeyStateCache(_QSAStateCache):
    ...
    def get_kv_cache_spec(self, vllm_config: VllmConfig) -> KVCacheSpec:
        ...
        # A circular-cache spec must also share a kernel block divisor with the
        # hybrid manager block. For example, MTP5 needs at least 9 rows: the
        # original whole-group rounding selects 12, which does not divide this
        # model's 3232-token manager block and has no common supported kernel
        # size. Advance by whole compression groups to the smallest divisor
        # (16 for MTP5..8), retaining extra history without changing semantics.
        while self.cache_config.block_size % capacity:
            capacity += self.compress_ratio
